  ...
  useEffect(() => {
    ...
    if (mapRef.current && naver && currentMyLocation) {
      ...
      const createMarker = (item, index) => {
        const postLocation = new naver.maps.LatLng(
      ...
      // 게시글 마커 표시
      userPost
        .filter((item) => activePin === "ALL" || item.postType === activePin)
        .forEach((item, index) => {
    ...
    activeMarker,
    clickedMarkers,
  ]); // `activePin` 및 `mapState`를 의존성 목록에 추가
    ...
    <div>
      ...
      >
        ...
        <button onClick={onClickAllPin} style={{ border: "none" }}>
          <img
            src={allPinState ? full_allPin : allPin}
            alt="allPin"
        ...
        <button onClick={onClickDailyPin} style={{ border: "none" }}>
          <img
            src={dailyPinState ? full_dailyPin : dailyPin}
            alt="dailyPin"
            style={{ width: "30px", height: "30px" }}
          />
        </button>
        <button onClick={onClickHistoryPin} style={{ border: "none" }}>
          <img
            src={historyPinState ? full_historyPin : historyPin}
            alt="historyPin"
            style={{ width: "30px", height: "30px" }}
          />
        </button>
